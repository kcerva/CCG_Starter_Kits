...
countries_short = country_data[country_data['Run_Asia2']==1]['Country']
icountries = country_data[country_data['Run_Asia2']==1]['ISO3']
...
# for ic, country in zip(icountries, countries_short):
    ...
    x2 = np.round(cfile_4['Efficiency (2050)'][0]*100,2)
    document.add_paragraph('The combined losses in electricity transmission and distribution in {0} are estimated based on an International Energy Agency (IEA) dataset presented by Index Mundi [13], which gives estimated combined losses in 2014. It was then assumed that combined losses would be reduced to 5% by 2050, falling in a linear fashion.  Combined transmission and distribution efficiency in {0} is therefore assumed to reach {1}% and {2}% in 2030 and 2050 respectively. The combined costs of power tansmission and distribution are estimated based on a report by the Economic Research Institute for ASEAN and East Asia (ERIA) [14], which gives cost estimates for several real-life projects in ASEAN. For more detail, see section 2.In the following table, the techno-economic parameters associated with the transmission and distribution network are presented.'.format(country,str(x1),str(x2)))

    # fix which table it takes
    document.add_paragraph('Table 4: Techno-economic parameters for transmission and distribution [13,14]')
    table = document.add_table(rows=1, cols=6)
    table.style = 'TableGrid'
    hdr_cells = table.rows[0].cells
    ...
    if dr.empty:
        dr = 0
    document.add_paragraph('{0} has an estimated {1} domestic refinery capacity [15]. In the OSeMOSYS model, two oil refinery technologies were made available for investment in the future, each with different output activity ratios for Heavy Fuel Oil (HFO) and Light Fuel Oil (LFO). The technoeconomic data for these technologies are shown in Table 5.'.format(country,str(dr)))
    # fix which table it takes
    document.add_paragraph('Table 5: Techno-economic parameters for refinery technologies [15,16]')
    table = document.add_table(rows=1, cols=5)
    table.style = 'TableGrid'
    ...
    document.add_paragraph('Assumed costs are provided for both imported and domestically-extracted fuels. The fuel price projections until 2050 are presented below. These are estimates based on Asia-specific cost estimates produced by the Asia Pacific Economic Cooperation (APEC) and ERIA [17.18], with an international average biomass price in 2020 assumed for imported biomass [19]. More detail is provided in Section 2.2.')

    # reduce number of decimals
    document.add_paragraph('Table 6: Fuel price projections to 2050 [17,18,19]')
    table = document.add_table(rows=1, cols=7)
    table.style = 'TableGrid'
    col0 = cfile_6.columns[0] 
    ...
    document.add_paragraph('Figure 2: Fuel price projections to 2050 [17,18,19]')
    ...
    document.add_paragraph('Table 7: Fuel-specific CO2 Emission Factors [20]')
    table = document.add_table(rows=1, cols=2)
    table.style = 'TableGrid'
    ...
    # reduce number of decimals
    document.add_paragraph('Table 8: Estimated Renewable Energy Potentials [12,21,22,23]')
    table = document.add_table(rows=1, cols=2)
    table.style = 'TableGrid'
    ...
    document.add_paragraph('Table 9: Estimated Fossil Fuel Reserves [17] ')
    table = document.add_table(rows=1, cols=2)
    table.style = 'TableGrid'
    # hdr_cells = table.rows[0].cells
    # hdr_cells[0].text = ''
    ...
    document.add_paragraph('Final electricity demand in {0} was estimated at {1} PJ in 2016 and is forecasted to reach {2} PJ by 2030 and {3} PJ by 2050 in a Business as Usual (BAU) scenario according to the APEC Energy Supply and Demand Outlook 7th Edition [17]. Figure 3 below shows the final electricity demand projection.'.format(country,str(e1),str(e2),str(e3)))

    # Add figure
    document.add_picture(r'C:/Users/lalli/Desktop/CCG/figures/{0}/Fig3_{1}.jpg'.format(country,ic), width=Inches(6))
    document.add_paragraph('Figure 3: Final Electricity Demand Projection (PJ) [17] ')
